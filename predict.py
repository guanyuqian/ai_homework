...
if __name__ == '__main__':
    ...
    def draw_image(left, right, mid, images_name):
        ...
        if (left):
            if constant.DEBUG: print(images_name + " left");
            draw.rectangle(constant.LEFT_AREA, None, 'red', width=10);
        if (mid):
            if constant.DEBUG: print(images_name + " mid");
            draw.rectangle(constant.MIDDLE_AREA, None, 'red', width=10);
        if (right):
            if constant.DEBUG: print(images_name + " right");
    ...
    Label(root, text="检测图片输入路径:").grid(row=0, column=0)
