...
def data_preprocessing():
    ...
    # 70%图像被当做训练集
    train_data = np.array(image_list[0:int(len(image_list)-200)])
    # 查看矩阵或者数组的维数
    train_label = np.array(y[0:int(len(image_list)-200)])
    train_label = train_label.reshape(train_label.shape[0], -1)
    # 验证集 剩下30%
    validation_data = np.array(image_list[int(len(image_list)-200):len(image_list)])
    # validation_data = validation_data.reshape(validation_data.shape[0], -1)
    validation_label = np.array(y[int(len(image_list)-200):len(image_list)])
